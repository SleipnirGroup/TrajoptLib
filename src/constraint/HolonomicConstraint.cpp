...
std::optional<SolutionError> HolonomicConstraint::CheckHolonomicState(
  ...
  if (IsVelocityConstraint()) {
    std::optional<SolutionError> check =
        GetVelocityConstraint().CheckVelocity(velocityX, velocityY, tolerances);
    if (check.has_value()) {
    ...
    std::optional<SolutionError> check =
        GetAngularVelocityConstraint().CheckAngularVelocity(angularVelocity,
                                                            tolerances);
    if (check.has_value()) {
      return SolutionError{
          fmt::format("({}) violated: {}", "*this", check->errorMessage)};
...
const AngularVelocityConstraint&
HolonomicConstraint::GetAngularVelocityConstraint() const {

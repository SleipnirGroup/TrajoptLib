...
std::optional<SolutionError> CheckState(
  ...
  if (std::holds_alternative<VelocityConstraint>(constraint)) {
    std::optional<SolutionError> check =
        std::get<VelocityConstraint>(constraint).CheckVelocity(velocityX, velocityY, tolerances);
    if (check.has_value()) {
    ...
    std::optional<SolutionError> check =
        std::get<AngularVelocityConstraint>(constraint).CheckAngularVelocity(angularVelocity,
                                                            tolerances);
    if (check.has_value()) {
      return SolutionError{
          fmt::format("({}) violated: {}", "*this", check->errorMessage)};

...
void adl_serializer<trajopt::HolonomicConstraint>::to_json(
    json& j,
    const trajopt::HolonomicConstraint& constraint) {
  if (const auto* _constraint =
    ...
    j.update(*_constraint);
      ...
      std::get_if<trajopt::HolonomicVelocityConstraint>(&constraint)) {
    j = json{{"constraintType", "holonomicVelocity"}};
    j.update(*_constraint);
  }
}

void adl_serializer<trajopt::HolonomicConstraint>::from_json(const json& j, trajopt::HolonomicConstraint& constraint) {

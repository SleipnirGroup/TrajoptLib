...
namespace helixtrajectory {
    ...
    HolonomicTrajectoryGenerator::HolonomicTrajectoryGenerator(const HolonomicDrive& drive, const HolonomicPath& path, const std::vector<Obstacle>& obstacles)
            : X(opti.variable(6, nTotal + 1)), x(X(0, all)), y(X(1, all)), theta(X(2, all)),
    ...
    std::unique_ptr<Trajectory> HolonomicTrajectoryGenerator::Generate() {
        opti.solver("ipopt");
    ...
    void HolonomicTrajectoryGenerator::ApplyBoundryConstraints() {
    ...
    void HolonomicTrajectoryGenerator::ApplyWaypointConstraints() {

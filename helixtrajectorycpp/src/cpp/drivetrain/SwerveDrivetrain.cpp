...
namespace helixtrajectory {
    ...
    //         std::array<double, 2> rotatedVelocity = ApplyRotation({state.velocityX, state.velocityY}, -state.heading);
    //         auto moduleVelocity = _module.CalculateVelocity(rotatedVelocity[0], rotatedVelocity[1], state.angularVelocity);
    //         double velocityMagnitudeSquared = moduleVelocity[0] * moduleVelocity[0] + moduleVelocity[1] * moduleVelocity[1];
    //         double maxVelocityMagnitude = _module.wheelMaxAngularVelocity * _module.wheelRadius;
    //         double maxVelocityMagnitudeSquared = maxVelocityMagnitude * maxVelocityMagnitude;

    //         if (!(velocityMagnitudeSquared <= maxVelocityMagnitudeSquared)) {
    //             throw IncompatibleTrajectoryException(fmt::format(
    //                     "wheel of module {} must have an angular velocity less than or equal to {}, but the trajectory requires {}.", moduleIndex, maxVelocityMagnitude, std::sqrt(velocityMagnitudeSquared)));
    //         }
    //     }
    // }

    // void SwerveDrivetrain::CheckTrajectory(const HolonomicTrajectory& trajectory) const {
    //     for (std::size_t sampleIndex = 0; sampleIndex < trajectory.samples.size() + 1; sampleIndex++) {

...
/**
 * @brief A certain state that the robot must have at some sample point of the
 * trajectory. Waypoint includes options to constrain dynamics like position and
 * velocity at the waypoint sample point. The waypoint sample point is defined
 * as the sample point associated with the waypoint. The trajectory segment, or
 * segment, is the ordered collection of sample points that lead up to the
 * waypoint, including the waypoint sample point but not including the previous
 * waypoint sample point in the path if it exists (if this is not the first
 * waypiont).
 */
class TRAJOPT_DLLEXPORT Waypoint {
 public:
  /**
   * @brief the constraint on position for this waypoint; these constraints are
   * applied only to the sample point associated with the waypoint
   */
  std::vector<Constraint> waypointConstraints;
  /**
   * @brief the constraint on position for this segment; these constraints are
   * applied to the sample points of the segment but are only additionally
   * applied to the waypoint sample point if applySegmentConstraintsToWaypoint
   * is set to true
   */
  std::vector<Constraint> segmentConstraints;

  /**
   * @brief the number of control intervals in the optimization problem from the
   * previous waypoint to this waypoint
   */
  size_t controlIntervalCount;
  /**
   * @brief the points used to construct the linear initial trajectory guess for
   * the trajectory segment from the last waypoint to this waypoint
   */
   ...
   *
   * @return true if the position state of the robot is constrained, false
   * otherwise
   */
   ...
   * before generation.
   *
   * @return true if the position and velocity state of the robot is
   ...
   *
   * @return true if and only if this waypoint is an initial waypoint or a known
   * state waypoint
   */
